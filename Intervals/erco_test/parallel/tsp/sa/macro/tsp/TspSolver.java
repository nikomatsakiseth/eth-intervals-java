...
public class TspSolver extends Thread {
	...
	for (;;) {
	    ...
	    curr = get_tour(curr);
	    if (curr < 0) {
		if (debug) System.out.println("Worker: curr = "+curr); 
		break;
	    }
	...
	if (debug) System.out.println("new_tour");
	...
	//if (debug) MakeTourString(curr.last, curr.prefix);
	...
	if (best >= MinTourLen) {
	    if (debug) System.out.println("set_best: "+best+" <-> "+MinTourLen);
	    return;
	}
	synchronized(MinLock) {
		...
		for (int i = 0; i < Tsp.TspSize; i++)
		    MinTour[i] = path[i];
		if (debug) MakeTourString(Tsp.TspSize, MinTour);
		//DumpPrioQ();
	    }
	}
    }
	...
	synchronized(TourLock) {
	if (debug) System.out.println("calc_bound");
	...
	synchronized(TourLock) {
	if (debug) System.out.println("split_tour");
	...
	synchronized(TourLock) {
	if (debug) System.out.println("find_solvable_tour");

	if (Done != 0) { if (debug) System.out.println("("+Thread.currentThread().getName()+") - done"); return -1; }
	...
	if (debug) System.out.println("get_tour");
	...
	if (debug) System.out.println("recursive_solve");
	...
	if (CurDist == 0 || debug) {
	    if (debug) System.out.print("\t"+Thread.currentThread().getName()+": Tour "+index+" is ");
	    for (i = 0; i < PathLen-1; i++) {
		System.out.print(Path[i]+" - ");
	    }
	    if (debug) System.out.println(Path[i]);
	    if (debug) System.out.println("\t"+Thread.currentThread().getName()+": Cur: "+CurDist+", Min "+MinTourLen +", Len:"+(PathLen-1)+", Sz: "+Tsp.TspSize+"\n");
	}
	...
	//System.out.print("visit_nodes - ");

	visitNodes++;

	for (i = 1; i < Tsp.TspSize; i++) {
	    if (Visit[i]!=0) continue;	/* Already visited. */
	    if ((dist = weights[from][i]) == 0) continue; /* Not connected. */
	    if (CurDist + dist > MinTourLen) continue; /* Path too long. */
	    ...
	    else if (CurDist < MinTourLen) visit_nodes(i);	/* Visit on. */
