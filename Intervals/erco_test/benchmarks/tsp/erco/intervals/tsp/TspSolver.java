...
public class TspSolver extends Interval {
	 ...
	 * new_tour():
	 * 
	 * Create a new tour structure given an existing tour structure and the next
		...
		return Intervals.subinterval(new SubintervalTask<Integer>() {
			...
			@Override public Integer run(Interval subinterval) {
				...
				for (i = 0; i < Tsp.TspSize; i++) {
					...
					curr.setConn(prev.conn());
				}
				curr.setLast(prev.last());
				curr.setPrefix_weight(prev.prefix_weight()
						+ weights[curr.prefix()[curr.last()]][move]);
				curr.prefix()[curr.setLast(curr.last() + 1)] = move;

				if (debug)
					MakeTourString(curr.last(), curr.prefix());

				curr.setConn(curr.conn() | (1 << move));
	 ...
	 */
	static void set_best(final int best, final int[] path) {
		//XXX assert MinLock.isReadable(); // XXX will fail
				...
				if (best < MinTourLen) {
					if (debug) {
						System.out.print("(" + Thread.currentThread().getName()
								+ ") ");
						System.out.println("set_best MinTourLen: " + best
								+ " (old: " + MinTourLen + ")");
					}
					MinTourLen = best;
	...
	/*
	 * Add up min edges connecting all unconnected vertixes (AHU p. 331-335) At
	 * some point, we may want to use MST to get a lower bound. This bound
	 * should be higher (and therefore more accurate) but will take longer to
	 * compute.
	 */
				...
				if (curr.last() == (Tsp.TspSize - 2)) {
					for (i = 0; i < Tsp.TspSize; i++) {
						if ((curr.conn() & (1 << i)) == 0) {
							...
							curr.setPrefix_weight(curr.prefix_weight()
									+ (weights[curr.prefix()[Tsp.TspSize - 2]][i]
							...
							curr.setLower_bound(Tsp.BIGINT);
				...
				if (curr.last() != Tsp.TspSize - 1) {
					for (i = 0; i < Tsp.TspSize; i++) {
						...
						for (j = 0, wt1 = wt2 = Tsp.BIGINT; j < Tsp.TspSize; j++) {
							...
							if ((wt == 0)
									|| (((curr.conn() & (1 << i)) != 0)
											&& (j != curr.prefix()[0]) && (j != curr.prefix()[curr.last()])))
								continue;
						...
						if (wt2 != Tsp.BIGINT)
							curr.setMst_weight(curr.mst_weight()
									+ ((wt1 + wt2) >> 1));
						/* Exactly two unconnected nodes? */
						else if (wt1 != Tsp.BIGINT)
							curr.setMst_weight(curr.mst_weight() + wt1);
					}
					curr.setMst_weight(curr.mst_weight() + 1);
				}
				curr.setLower_bound(curr.mst_weight() + curr.prefix_weight());
				return (curr_index);
			}
		});
	}
	...
	static void DumpPrioQ() {
		...
		for (pq = 1; pq <= PrioQLast; pq++) {
			...
			if ((LEFT_CHILD(pq) < PrioQLast)
					&& (PrioQ[pq].priority > PrioQ[LEFT_CHILD(pq)].priority))
				System.out.print(" left child wrong! ");
			if ((LEFT_CHILD(pq) < PrioQLast)
					&& (PrioQ[pq].priority > PrioQ[RIGHT_CHILD(pq)].priority))
				...
				if (curr.last() != (Tsp.TspSize - 1)) {
					...
					for (i = 0; i < Tsp.TspSize; i++) {
						...
						if (t1 && t2 && t3) {
							...
							/*
							 * Bubble the entry up to the appropriate level to
							 * maintain the invariant that a parent is less than
							 * either of it's children.
							 */
							for (parent = pq >> 1, pptr = PrioQ[parent]; (pq > 1)
									&& less_than(cptr, pptr); pq = parent, cptr = pptr, parent = pq >> 1, pptr = PrioQ[parent]) {
								/*
								 * PrioQ[pq] lower priority than parent -> SWITCH
								 * THEM.
								 */
								index = cptr.index;
							...
							System.out.println(" [" + curr.lower_bound() + " + " + wt
									+ " > " + MinTourLen);
					}
				}				
			}
			...
			@Override public Integer run(Interval subinterval) {
				...
				for (; PrioQLast != 0;) {
					...
					if (pptr.priority >= MinTourLen) {
						/*
						 * We're done -- there's no way a better tour could be
						 * found.
						 */
						if (debug) {
							System.out.print("\t"
					...
					/*
					 * Bubble everything maintain the priority queue's heap
					 * invariant.
					 */
					...
					/*
					 * Push previous last element down tree to restore heap
					 * structure.
					 */
					for (i = 1; i <= (PrioQLast >> 1);) {
						...
						if (less_than(cptr, pptr)) {
							/*
							 * PrioQ[child] has lower prio than its parent - switch
							 * 'em.
							 */
							index = pptr.index;
					...
					if (last < Tsp.TspSize || last < 1) {
						if (last >= (Tsp.TspSize - Tsp.NodesFromEnd - 1))
							return (curr);
						else
							split_tour(curr); /* The current tour is too long, */
	...
	static int get_tour(final int curr) {
	 ...
	 * 
	 * We're now supposed to recursively try all possible solutions starting
	 * with the current tour. We do this by copying the state to local variables
	 * (to avoid unneeded conflicts) and calling visit_nodes to do the actual
	 * recursive solution.
	 */
