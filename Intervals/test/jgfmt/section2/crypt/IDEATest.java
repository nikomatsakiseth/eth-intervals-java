...
class IDEATest {
	...
	void Do() {
		...
		if(JGFCryptBench.nthreads == -1) {
			...
			thobjects[0] = new IDEARunner(0, crypt1, plain2, DK);
			thobjects[0].run();
	 ...
	 * mul
	 * 
	 * Performs multiplication, modulo (216)+1. This code is structured on the
	 ...
	 * int and one 64-bit long to keep 16-bit values and their products
	 * "unsigned." The routine assumes that both a and b could fit in 16 bits
	 * even though they come in as 32-bit ints. Lots of "& 0xFFFF" masks here to
	 * keep things 16-bit. Also, because the routine stores mod (216)+1 results
	 * in a 216 space, the result is truncated to zero whenever the result would
	 * zero, be 216. And if one of the multiplicands is 0, the result is not
	 * zero, but (216) + 1 minus the other multiplicand (sort of an additive
	 * inverse mod 0x10001).
	 * 
	 ...
	 * inv
	 * 
	 * Compute multiplicative inverse of x, modulo (216)+1 using extended
...
class IDEARunner implements Runnable {
	...
	public void run() {
		...
		ttslice = (tslice + JGFCryptBench.nthreads - 1)
				/ JGFCryptBench.nthreads;
		slice = ttslice * 8;
